...
_FALLBACK_EXTRA = [
    ...
    # последний рубеж — бесплатная модель. вся цепочка выше платная и на одном счёте:
    # кончились деньги — психолог молчал в ответ на «не хочу жить». хуже качеством,
    # но живой ответ несравнимо лучше строки «все модели не ответили».
    #
    # Именно эта, а не авто-роутер openrouter/free: тот уводит на случайную свободную
    # модель, в том числе на классификатор безопасности, и на «не хочу жить» отвечает
    # «User Safety: unsafe». Проверено на кризисной реплике: gemma остаётся рядом,
    # держит строчные и даёт телефон; nemotron вываливает свои размышления в ответ.
